// console.log(steg.encode('Hello How Are You?' , 'data:image/gif;base64,'  ));

// console.log(steg.decode('data:image/png;base64,'));

var auddatauri;

function readURL(input) {
  if (input.files && input.files[0]) {
    var reader = new FileReader();

    reader.onload = function(e) {
      document.querySelector("audio1").src = e.target.result;
      auddatauri = e.target.result;
    };
  }
  reader.readAsDataURL(input.files[0]);
}


function decode(input) {
    if (input.files && input.files[0]) {
      var reader = new FileReader();
  
      reader.onload = function(e) {
          console.log(steg.decode(e.target.result));
        
        document.querySelector('#decoded').innerText = steg.decode(e.target.result);
      };
    }
    reader.readAsDataURL(input.files[0]);
  }


function hideText() {
  document.querySelector("audio2").src = steg.encode(document.querySelector('#text').value, auddatauri);
}