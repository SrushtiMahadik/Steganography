// console.log(steg.encode('Hello How Are You?' , 'data:image/gif;base64,'  ));

// console.log(steg.decode('data:image/png;base64,'));

var viddatauri;

function readURL(input) {
  if (input.files && input.files[0]) {
    var reader = new FileReader();

    reader.onload = function(e) {
      document.querySelector("#video1").src = e.target.result;
      viddatauri = e.target.result;
    };
  }
  reader.readAsDataURL(input.files[0]);
}

function decode(input) {
    if (input.files && input.files[0]) {
      var reader = new FileReader();
  
      reader.onload = function(e) {
          console.log(steg.decode(e.target.result));
          document.querySelector("#decoded").innerText= steg.decode(e.target.result);
          //viddatauri = e.target.result;
       // document.querySelector('#decoded').innerText = steg.decode(e.target.result);
      };
    }
    reader.readAsDataURL(input.files[0]);
  }

function hideText() {
 document.querySelector("#video2").src = steg.encode(document.querySelector('#text').value, viddatauri);
// document.querySelector('#video2').innerText = steg.readURL(e.target.result);
//<video id="video2" src="data:video/mp4;base64,AAAAI05178AAA5zcbv7+AACgFVSDADA-" alt=""></video>
}